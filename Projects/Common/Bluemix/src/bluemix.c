...
/* Definitions added due to integration conflict with #include "main.h" */
...
int PrepareToggleMqttPayload(char * PayloadBuffer, int PayloadSize, char * deviceID, int toggle)
{
  ...
  snprintfreturn = snprintf( Buff, BuffSize, "{\"d\":{\"Led state\": %d, \"Timestamp\": \"%s\", \"pressure\": %.2f}}", toggle, timestamp, p );
...
void TestSensorPublish(device_info_t *dev)
{
  ...
  if (is_quickstart) {
    ...
    sprintf(nfcUrl, "https://%s.internetofthings.ibmcloud.com/#/device/%s/sensor/", dev->orgid,dev->device_id);
    if(nfc04Status==NFCTAG_OK)
        WriteURLtoNFC(nfcUrl);
    printf("\nYou can see your published data at %s\n\r", nfcUrl);        
    //printf("Device Id is %s\n\n\r", dev->device_id);
  }
  else {
    /* Info to the user in order to access to the device data via IBM Watson IoT Platform */
    nfcUrl[0] = 0;
    sprintf(nfcUrl, "https://%s.internetofthings.ibmcloud.com", dev->orgid);   
    if(nfc04Status==NFCTAG_OK)
        WriteURLtoNFC(nfcUrl);    
    printf("\nYou can see your published data at %s\n\r", nfcUrl);        
    printf("Sign in. ");
...
#if (defined FFT_DEMO)    
    if (command == BP_TRIPLE_PUSH)                  /* If tiple button push, toggle the telemetry publication. */
    {
      // first set the button flag to zero, than proceed further
      button_flags = 0;
      msg_info("Exiting the test. Initiating de-initialize procedure.\n");      
      break;
    }    
#endif //   FFT_DEMO  
...
/**
  * @brief  Run the Bluemix test. The devide credentials including the registration mode are retrieved from flash
  * @param  none
  * @retval 0:    the connection string in Flash has been parsed succesfully and the output structure fields have been set
  *        -1:    in case of failure
  */
